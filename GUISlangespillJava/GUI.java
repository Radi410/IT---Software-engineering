...
public class GUI {
    ...
    public void tegnStart(){
        ...
        for(int i = 0; i < antSkatter; i++){
            ...
            kontroller.addSkatt(i, ny);
        }
    }
    public void tegnIgjen(){
        Koe<Slange> slange = kontroller.getSlange();
        Skatt[] skattene = kontroller.getSkatter();
        ...
        for(int i = 0; i < skattene.length; i++){
            if(skattene[i] != null){
                ruter[skattene[i].getRadnr()][skattene[i].getKolnr()].setText("$");
            }
        }
        for(Slange part : slange){
            int slangeRad = part.getRadnr();
            int slangeKol = part.getKolnr();
            if(part.erHode()){
        ...
        snakelengde.setText(" " + kontroller.getHalelengde() + " ");
