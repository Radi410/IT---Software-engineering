...
class Legesystem{
  ...
  public void statistikk(){  
    ...
    system.out.println("Antall utskrevene resepter for narkotisk:" + tellerN);
    system.out.println("Antall utskrevene resepter for Vanedammende:" + tellerV);
    if(r.hentLegemiddel() instanceof Narkotisk){
      Prioritetskoe<Lege> narkotiskeLeger = new Prioritetskoe<>();
      int antallNarkotiskPerlege = 0;
      for (Resept r : resepter){
        if (r.hentLegemiddel()instanceof Narkotisk){
          Lege narkotiskLege = r.hentLege();
          narkotiskeLeger.leggTil(narkotiskLege);
        }
      }
      system.out.println(narkotiskeLeger + " antall narkotiske resepter skrevet ut:" + antallNarkotiskPerlege);

      IndeksertListe<Pasient> narkotiskePasienter = new IndeksertListe<>();
      for (Resept res : resepter){
        if (res.hentLegemiddel()instanceof Narkotisk){
          Pasient narkotiskPasient = res.hentPasient();
          narkotiskePasienter =  res.hentPasient();
          narkotiskePasienter.leggTil(narkotiskPasient);
          }

        }
        System.out.println("Pasienter som har minst en respet av narkotisk legemiddel"+ "\n");
        for (Pasient p : narkotiskePasienter){
          System.out.println("Pasientnavn:"+ p.hentNavn()+ "antall resepter narkotisk"+ p.antallNarkotiskeLegemidler());
        }
